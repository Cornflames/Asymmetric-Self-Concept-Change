...
In dieser Datei wird eine vollständige Re-Implementation Reproduction von Studie 4 in Brotzeller & Gollwitzer (2025) nach den Schritten durchgeführt, die das *Conceptual Framework for Computational Reproductions* (Kohrt et al., 2024) vorgibt. Eine besser lesbare HTML-Version dieser Datei findet sich in den [Zusatzmaterialien](https://github.com/Cornflames/Asymmetric-Self-Concept-Change.git) der vorliegenden Bachelorarbeit.

## (a) Defining the Scope

-   **Works**: Studie 4 aus Brotzeller & Gollwitzer (2025)
-   **Claims**:
...
-   **Results**: Tabelle 2 auf S. 1739. Diese Tabelle bildet alle Koeffizienten des Regressionsmodells mit den Prädiktoren `size of discrepancy (SoD)`, `direction of discrepancy (DoD)` und `opportunity for improvement (OFI)` sowie all ihren Interaktionseffekten ab. Das Modell sagt die absoluten Selbstkonzeptänderungen vorher.
-   **Obtained data**: Anonymisierte Primärdaten, die [hier](https://osf.io/yadqw) aus dem OSF abrufbar sind.
    -   Die Primärdaten wurden schon so weit vorverarbeitet, dass Prädiktorvariablen des Regressionmodells bereits aus anderen Variablen im Datensatz berechnet wurden. Dieser Verarbeitungsschritt wird wiederholt.
...
-   **Involved descriptions**: Die Beschreibungen aus den folgenden Quellen werden miteinbezogen und auf etwaige Inkohärenzen überprüft:
    ...
    -   Beschreibungen der Berechnung der für die Regressionsanalyse benötigten Variablen im Codebook
    -   Für die Formel zur Berechnung der Diskrepanz zwischen Feedback und Selbstwahrnehmung musste auf eine Beschreibung in den Materialien zu Studie 4 zurückgegriffen werden
...
-   Die anonymisierten [Primärdaten](https://osf.io/yadqw) konnten abgerufen und eingebunden werden:
...
::: callout-tip
## Fazit: Auschluss von Versuchspersonen erfolgreich reproduziert

Im Einklang mit der von Brotzeller & Gollwitzer (2025) berichten Anzahl, wurden n = `r nrow(original_data) - nrow(data_after_exclusion)` Personen von der Analyse ausgeschlossen.
:::
...
Weil das Feedback als Prozentwert vorliegt, muss die Differenz ebenfalls zu einem Prozentwert umgerechnet werden. Dazu müssen die Selbstwahrnehmungswerte von der ursprünglichen Skala von 1-9 auf die Skala 0-8 umgerechnet werden, die einen Nullpunkt hat. Das Feedback errechnet sich als prozentualer Gesamtwert aus den einzelnen Items des Reading the Mind in the Eyes Test (Bölte, 2005); diese Berechnung wird nicht noch einmal wiederholt, sondern von den Autoren übernommen.
...
Folgt man den Beschreibungen von Brotzeller & Gollwitzer (2025) bei der Berechnung von `SoD` und `DoD`, stimmen die Werte nicht mit jenen überein, die die Autoren in ihrem Datensatz berechnet haben.
...
Nachdem Mistake 1 gelöst wurde und Obstacle 1 überwunden wurde, stimmen die Werte der neu berechneten Variablen mit den ursprünglichen Daten überein:
...
Es fällt auf, dass der `lm`-Befehl für die einzelnen Koeffizienten nicht die Effektstärken $sr^2$ (squared semi-partial correlations) berechnet. Da Brotzeller & Gollwitzer (2025) diese Effektstärken angeben, müssen sie nachträglich berechnet werden.
...
Ein maueller Vergleich der neuberechneten mit den berichteten Ergebnissen zeigt, dass die Reproduktion **konsistente** Ergebnisse hervorgebracht hat. Rundet man die neuberechneten Werte auf zwei Nachkommastellen, sind sie sogar **numerisch identisch** zu den in Tabelle 2 (S. 1739) enthaltenen Werten sind.
:::
